...
# exclusive for wsl, warn when using windows file system
function zsh_win_status {
    WIN_STATUS=""

    if [[ "$PWD" =~ ^/mnt ]]; then
        WIN_STATUS=" %{$fg[magenta]%}[ win]%{$reset_color%}"
...
PROMPT='${ZSH_ESSEMBEH_PREFIX}%{$fg_bold[$ZSH_ESSEMBEH_COLOR]%}%n@%M%{$reset_color%}:%{%B$fg[blue]%}%~$(zsh_win_status)%{$reset_color%b%}$(zsh_essembeh_gitstatus)%(!.#.$) '
